 ...
 * Account storage.
 *
 * IndexedDB rather than localStorage: a novel runs to hundreds of thousands
 * of characters, and localStorage's ~5MB ceiling is a real risk once you
 * have a few books. IndexedDB gives orders of magnitude more room, and the
 * browser is far less eager to evict it.
 *
 * Everything is device-local. There is no server, so an account does not
 * follow you to another browser or device — that is what the backup file in
 * Settings is for.
 */
...
/**
 * Asks the browser to keep this data even under storage pressure. Without
 * it, a browser clearing space can evict IndexedDB — which for this app
 * means someone's manuscript. Silently ignored where unsupported.
 */
...
export function getAccount(username: string): Promise<AccountRecord | undefined> {
  return tx<AccountRecord | undefined>(ACCOUNTS, "readonly", (s) => s.get(username));
}

export async function createAccount(username: string, password: string): Promise<CryptoKey> {
  const existing = await getAccount(username);
  if (existing) throw new Error("That name is already used on this device.");
...
export async function login(username: string, password: string): Promise<CryptoKey | null> {
...
export async function changePassword(
  ...
  await tx(ACCOUNTS, "readwrite", (s) => s.put({ ...account, salt: toBase64(salt), verifier }));
  ...
  setActive(username, nextKey);
  return true;
}

export async function readVault(username: string, key: CryptoKey): Promise<string | null> {
...
export async function setSession(username: string, key: CryptoKey): Promise<void> {
  try {
...
export async function getSession(): Promise<{ username: string; key: CryptoKey } | null> {
  try {
...
export function getActive(): { username: string; key: CryptoKey } | null {
  return activeKey && activeUser ? { username: activeUser, key: activeKey } : null;
}
